...
int main()
{
	...
	while (1)
	{
		...
		while (!bDead)
		{
			...
			fDeltaTime = std::chrono::duration<float>(tp2 - tp1).count();
			tp1		   = tp2;
			...
			for (int x = 0; x < nBlocksWidth; x++)
				for (int y = 0; y < nBlocksHeight; y++)
				{
					...
					if (b > 0)
						screen[(y + nBlockOffsetY) * nScreenWidth + (x + nBlockOffsetX)] = std::to_wstring(b).c_str()[0];
				}
